 ...
 */
@Path("/jpgtobmp")
public class JPGtoBMPRestEndPoint<D extends ConfiguredEndpointDefinition> extends AbstractEndpoint<D> {
    ...
    private static final Logger log = LoggerFactory.getLogger(JPGtoBMPRestEndPoint.class);

    @Inject
    public JPGtoBMPRestEndPoint(final D endpointDefinition) {
    ...
    public Response getNewsByNodeName(@QueryParam("id") String id, @QueryParam("name") String name) throws RepositoryException {
        ...
        String filePathPng = PATH + id + DELIMITER + nameWithoutExtension + ".bmp" ;
        ...
        System.out.println("That took " + (endTime - startTime) + " milliseconds for converting from JPG to BMP");
